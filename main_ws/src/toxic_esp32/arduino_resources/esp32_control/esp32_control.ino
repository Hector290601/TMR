...
#define SERVO_PIN 19 //Pin del servo
...
//----[Funciones]----
/*
void blinker: hace titilar el led en un cierto espacio de tiempo que nosotros requiramos en este caso segundos que mediante una formula podemos convertilo a hertz
*/
void blinker1(){
...
/*
void servocontrol (float grados): Mueve nuestro servor cierta cantidad de grados
*/
void servocontrol (float grado){
  //Espera un segundo para que se concrete la operacion
  if (millis() - timing; 1000){ 
      timing = millis(); 
    servoPrincipal.write(grado);
 }
}
...
{ //Ejecutando nuestro const void para que sea del mismo tipo de msgs mediante un cast
  ...
  float pin=msg->data.data[0];
  int selector= (int) pin;
  float frecseg=0;
  
  //Selector de casos
  switch(selector){
    case 0:
      frecseg= hz_to_seg(msg->data.data[1]);
      blink1.detach();
...
void setup() {
  ...
  servoPrincipal.attach(SERVO_PIN,500,2500);
  ...
  //Posicion inicial de servo
  servoPrincipal.write(25);
...
//---->Configurando nuestro subscriber
  ...
  RCCHECK(rclc_node_init_default(&node, "esp32_control", "", &support));
