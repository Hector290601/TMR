...
import cv2 #módulo para la visión artificial
    ...
    if lines is not None: #verifica que existan líneas
        for x1, y1, x2, y2 in lines: #itera en cada línea
            ...
            try: #evita que el programa falle si no hay líneas
                cv2.line(lineImage, (x1, y1), (x2, y2), (255, 0, 0), 10) #dibuja una línea azul en las líneas
            except:
...
try:
    while cap.isOpened():
        try:
            ...
            lines = cv2.HoughLinesP(colorZone, 2, np.pi / 180, 100, np.array([]), minLineLength = 10, maxLineGap = 50) #busca las líneas, los últimos dos valores definen el grosor mínimo y la distancia entre pixeles máxima, respectivamente
            if lines is not None: #verifica que existan líneas
                ...
                if (l2x2 - l2x1) != 0: #evita divisiones entre 0
                    ...
                    #print('mD: ', mD)
                #cv2.line(laneIMage, (mx1, 0), (mx1, 300), (0, 0, 0), 1) #dibuja una línea
                ...
                cv2.line(laneIMage, (my1, 0), (my1, 300), (0, 0, 255), 1) #dibuja una línea
